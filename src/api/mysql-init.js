...
export default async function handler(req, res) {
  ...
  await db.doQuery(
    connection,
    `INSERT INTO users (id,username,fullname,password) VALUES(1000,'user@test.com', 'User Test', '${pbkdf2_hash}')`
  )
  ...
  await db.doQuery(
    connection,
    `INSERT INTO documents (id,filename) VALUES(1,'file1.txt')`
  )
  await db.doQuery(
    connection,
    `INSERT INTO documents (id,filename) VALUES(2,'file2.txt')`
  )
  await db.doQuery(
    connection,
    `INSERT INTO documents (id,filename) VALUES(3,'file3.txt')`
  )
  await db.doQuery(
    connection,
    `INSERT INTO documents (id,filename) VALUES(4,'file4.txt')`
  )
  await db.doQuery(
    connection,
    `INSERT INTO documents (id,filename) VALUES(5,'file5.txt')`
  )
  ...
  await db.doQuery(
    connection,
    `INSERT INTO userdocs (id_users,id_documents) VALUES(1000,1)`
  )
  await db.doQuery(
    connection,
    `INSERT INTO userdocs (id_users,id_documents) VALUES(1000,2)`
  )
